...
Send the token and nonce to your backend and verify using `phonelink/validate`. See the [Server Verification](/validate) guide for the full walkthrough.
...
- **Single-use nonce** — `getResult` removes the nonce from storage after reading it. Calling it twice will return `null` the second time.
