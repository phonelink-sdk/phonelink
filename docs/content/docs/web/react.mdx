...
The hook uses the same `phonelink.verify` and `phonelink.getResult` methods as the [vanilla JS integration](/web/vanilla), wrapped in React state management.
...
After receiving the result on your frontend, always verify the token on your server. See the [Server Verification](/validate) guide.
