...
The hook uses the same `phonelink.startAuth` and `phonelink.handleCallback` methods as the [vanilla JS integration](/docs/web/vanilla), wrapped in React state management.
...
After receiving the result on your frontend, always verify the token on your server. See the [Server Verification](/docs/server) guide.
