...
Every Phonelink integration — whether web, React, or Expo — requires **server-side token verification**. The client never verifies tokens; it passes the raw JWT and nonce to your server, where `verifyPhonelinkToken` performs cryptographic validation.
...
## What `verifyPhonelinkToken` checks
...
Always wrap `verifyPhonelinkToken` in a try/catch. The function throws for any validation failure:
