...
- The nonce was already consumed by a previous `handleCallback()` call (e.g. the page was refreshed)
...
- Don't call `handleCallback()` more than once per flow
...
**Fix:** Verify that the client ID you pass to `verifyPhonelinkToken` exactly matches the one used in `startAuth` or `phonelink.verify` on the client.
...
- The nonce was already consumed by a previous `handleCallback()` call
...
- Only call `handleCallback()` once per page load
- Make sure the callback URL in `startAuth` matches the page where `handleCallback` runs
...
- `startAuth("client-id", ...)` on the client
- `verifyPhonelinkToken(token, nonce, "client-id")` on the server
...
While the token itself doesn't become invalid after one use, the nonce is consumed on the client side after a single `handleCallback()` call. For server-side deduplication, you can track the `jti` (JWT ID) claim to prevent processing the same token twice.
