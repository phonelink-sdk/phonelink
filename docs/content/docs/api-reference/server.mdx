---
title: phonelink/server
description: API reference for the Phonelink server-side token verification.
---

## Exports

The `phonelink/server` entry point exports the following:

| Export | Type | Description |
| --- | --- | --- |
| `verifyPhonelinkToken` | `function` | Verifies a Phonelink JWT token |
| `PhonelinkPayload` | `type` | TypeScript type for the decoded JWT payload |

---

## `verifyPhonelinkToken(token, expectedNonce, expectedAud)`
...
TypeScript interface for the decoded JWT payload returned by `verifyPhonelinkToken`.
...
```ts
import type { PhonelinkPayload } from "phonelink/server";
```
